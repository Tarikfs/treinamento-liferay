...
public class AssignmentPersistenceImpl
	...
	private FinderPath _finderPathFetchByCollection;
	private FinderPath _finderPathCountByCollection;

	/**
	 * Returns the assignment where groupId = &#63; or throws a <code>NoSuchAssignmentException</code> if it could not be found.
	 *
	 * @param groupId the group ID
	 * @return the matching assignment
	 * @throws NoSuchAssignmentException if a matching assignment could not be found
	 */
	@Override
	public Assignment findByCollection(long groupId)
		throws NoSuchAssignmentException {

		Assignment assignment = fetchByCollection(groupId);
	...
	/**
	 * Returns the assignment where groupId = &#63; or returns <code>null</code> if it could not be found. Uses the finder cache.
	 *
	 * @param groupId the group ID
	 * @return the matching assignment, or <code>null</code> if a matching assignment could not be found
	 */
	@Override
	public Assignment fetchByCollection(long groupId) {
		return fetchByCollection(groupId, true);
	}

	/**
	 * Returns the assignment where groupId = &#63; or returns <code>null</code> if it could not be found, optionally using the finder cache.
	 *
	 * @param groupId the group ID
	 * @param useFinderCache whether to use the finder cache
	 * @return the matching assignment, or <code>null</code> if a matching assignment could not be found
	 */
	@Override
	public Assignment fetchByCollection(long groupId, boolean useFinderCache) {
		Object[] finderArgs = null;
		...
		if (useFinderCache) {
			result = finderCache.getResult(
				_finderPathFetchByCollection, finderArgs, this);
		}
		...
		if (result == null) {
			...
			sb.append(_FINDER_COLUMN_COLLECTION_GROUPID_2);
			...
			try {
				...
				if (list.isEmpty()) {
					if (useFinderCache) {
						finderCache.putResult(
							_finderPathFetchByCollection, finderArgs, list);
					}
				}
				else {
					if (list.size() > 1) {
						...
						if (_log.isWarnEnabled()) {
							...
							_log.warn(
								"AssignmentPersistenceImpl.fetchByCollection(long, boolean) with parameters (" +
	...
	/**
	 * Removes the assignment where groupId = &#63; from the database.
	 *
	 * @param groupId the group ID
	 * @return the assignment that was removed
	 */
	@Override
	public Assignment removeByCollection(long groupId)
		throws NoSuchAssignmentException {

		Assignment assignment = findByCollection(groupId);
	 ...
	 */
	@Override
	public int countByCollection(long groupId) {
	...
	private static final String _FINDER_COLUMN_COLLECTION_GROUPID_2 =
		"assignment.groupId = ?";
	...
	public void cacheResult(Assignment assignment) {
		...
		finderCache.putResult(
			_finderPathFetchByCollection,
			new Object[] {assignment.getGroupId()}, assignment);
	}
	...
	protected void cacheUniqueFindersCache(
		...
		finderCache.putResult(
			_finderPathCountByCollection, args, Long.valueOf(1));
		finderCache.putResult(
			_finderPathFetchByCollection, args, assignmentModelImpl);
	}
	...
	public void activate() {
		...
		_finderPathFetchByCollection = new FinderPath(
			FINDER_CLASS_NAME_ENTITY, "fetchByCollection",
